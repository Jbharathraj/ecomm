...
const SearchForm = () => {
  ...
  const handleSearch = () => {
    const filtered = data.filter((item) => {
      ...
      const matchesCategory = !categoryQuery || item.category.includes(categoryQuery);
      ...
      <div className="results">
        {results.length > 0 ? (
          <ul className="results-grid">
            {results.map((item) => (
              <li key={item.id}>
                <strong>{item.id}</strong> — {item.title} (Color: {item.color.join(", ")}, Material: {item.material}, Price: ₹
                {item.price}, Size: {item.size.join(", ")}, Rating: {item.rating}, Category: {item.category})
              </li>
            ))}
          </ul>
        ) : (
